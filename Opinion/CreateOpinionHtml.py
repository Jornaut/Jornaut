...
class CreateOpinionHtml:
    def __init__(self, html, title, text):
        self.html = html
        self.title = title
        GetAudioFile(str(title).replace(" ", "-").replace("'", "").replace('"', "").replace("|", ""), text)
        
        self.SaveHtml()
    def SaveHtml(self):
        self.file = open("c:/xampp/htdocs/jornaut/opinion/"+str(self.title).replace(" ", "-").replace("'", "").replace('"', "").replace("|", "")+"OpEd"+".php","w", encoding='utf-8')
